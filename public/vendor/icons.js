// Minimal inline icon set, sourced from Lucide (ISC license, lucide.dev).
// Each entry is the inner markup of a 24x24 stroke icon; wrapped at render
// time by icon(name, size) so callers don't need a build step or network.
window.ICONS = {
...
window.icon = function icon(name, size) {
  ...
  return `<svg class="i i-${name}" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${inner}</svg>`;
};
