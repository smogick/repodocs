...
const { REPO_ROOT, findEntityByPath } = require('./scan');

const TRASH_DIR = path.join(REPO_ROOT, 'trash');
...
function softDelete(displayPath) {
  ...
  // entities (flat resource files) entity.dir === REPO_ROOT/rootName and the
  // real file lives at entity.dir/entity.mainFile.
  const isFlatFile = entity.baseDir === entity.rootName;
...
function restore(trashId) {
  ...
  const destDir = path.join(REPO_ROOT, meta.rootName);
